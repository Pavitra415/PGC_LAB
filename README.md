...
Matrix multiplication is an operation where two matrices are multiplied to produce a third matrix.

### 2. Why is matrix multiplication suitable for parallel computing?

Many elements of the output matrix can be calculated independently, so the calculations can be performed simultaneously.

### 3. What is OpenMP?

OpenMP is an API used for parallel programming on shared-memory multi-core CPUs.

### 4. What is MPI?

MPI stands for Message Passing Interface. It allows processes to communicate and exchange data in distributed-memory systems.

### 5. What is CUDA?

CUDA is NVIDIA's parallel computing platform that allows programs to execute computations on NVIDIA GPUs.

### 6. What is speedup?

Speedup measures how much faster a parallel implementation is compared with the sequential implementation.

```text
Speedup = Sequential Time / Parallel Time
```

### 7. Why is Sequential used as the baseline?

Sequential execution provides a reference time for calculating the speedup of the parallel implementations.

### 8. What is MPI_Scatter?

`MPI_Scatter` distributes portions of data from one process to multiple processes.

### 9. What is MPI_Bcast?

`MPI_Bcast` broadcasts the same data from one process to all other processes.

### 10. What is MPI_Gather?

`MPI_Gather` collects data from multiple processes into one process.

### 11. What is a CUDA kernel?

A CUDA kernel is a function that is executed by GPU threads.

### 12. Why is CUDA suitable for matrix multiplication?

Matrix multiplication contains many independent calculations, allowing thousands of GPU threads to work in parallel.

### 13. What is the verification result?

```text
C[0][0] = 4000.00
```

### 14. What is the difference between OpenMP and MPI?

OpenMP generally uses shared memory and multiple threads, while MPI uses separate processes that communicate through message passing.

### 15. What is the main advantage of CUDA?

CUDA provides massive parallel execution using GPU threads for suitable computational workloads.
...
Sequential Time   : 244.12 seconds
OpenMP Time       : 30.83 seconds
MPI Time          : 92.98 seconds
CUDA Time         : 0.165 seconds

Sequential Speedup: 1.00×
OpenMP Speedup    : 7.92×
MPI Speedup       : 2.63×
CUDA Speedup      : 1479.48×
